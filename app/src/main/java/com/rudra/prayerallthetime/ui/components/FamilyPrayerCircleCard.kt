...
fun FamilyPrayerCircleCard(onClick: () -> Unit) {
    val familyMembers = listOf(
        FamilyMember("You", 5),
        FamilyMember("Father", 4),
        FamilyMember("Mother", 5),
        FamilyMember("Sister", 3)
    )
...
fun FamilyMemberProgress(member: FamilyMember) {
    Row(modifier = Modifier.padding(vertical = 4.dp)) {
        ...
        LinearProgressIndicator(
            progress = member.prayersCompleted.toFloat() / member.totalPrayers,
            modifier = Modifier.weight(1f)
        )
        Text(
            text = "${member.prayersCompleted}/${member.totalPrayers}",
